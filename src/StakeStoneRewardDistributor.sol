...
contract StakeStoneRewardDistributor is AccessControl {
    ...
    function claim(
        ...
        for (uint256 i=0; i<length; i++) {
            ...
            bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(msg.sender, token, totalClaimableAmount))));
            require(MerkleProof.verify(_proof[i], root, leaf), "Invalid proof");
            claimed[msg.sender][token] += claimAmount;
            require(claimed[msg.sender][token] <= totalClaimableAmount, "Invalid amount");
